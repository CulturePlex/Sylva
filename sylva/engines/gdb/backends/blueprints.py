...
class BlueprintsGraphDatabase(BaseGraphDatabase):
    ...
    def _check_property(self, element, key):
    ...
    def _set_element_label(self, element, label):
    ...
    def delete_node(self, id):
        vertex = self._get_vertex(id)
        self.gdb.removeVertex(vertex)

    def get_node_label(self, id):
        vertex = self._get_vertex(id)
        return self._get_element_label(vertex)

    def set_node_label(self, id, label):
        vertex = self._get_vertex(id)
        return self._set_element_label(vertex, label)

    def get_node_property(self, id, key):
        vertex = self._get_vertex(id)
        return self._get_element_property(vertex, key)

    def set_node_property(self, id, key, value):
        vertex = self._get_vertex(id)
        self._set_element_property(vertex, key, value, element_type=VERTEX)

    def delete_node_property(self, id, key):
        vertex = self._get_vertex(id)
        self._delete_element_property(vertex, key, element_type=VERTEX)

    def get_node_properties(self, id):
        vertex = self._get_vertex(id)
        return self._get_public_properties(vertex)

    def set_node_properties(self, id, properties):
        vertex = self._get_vertex(id)
        self._set_element_properties(vertex, properties, element_type=VERTEX)

    def update_node_properties(self, id, properties):
        vertex = self._get_vertex(id)
        self._update_element_properties(vertex, properties,
                                        element_type=VERTEX)

    def delete_node_properties(self, id):
        vertex = self._get_vertex(id)
        self._delete_element_properties(vertex, element_type=VERTEX)
    ...
    def _yield_nodes(self, nodes, include_properties):
        for node in nodes:
            if include_properties:
    ...
    def get_all_nodes(self, include_properties=False):
        return self._yield_nodes(
            self.node_index.get("graph", unicode(self.graph_id)),
            include_properties)

    def get_nodes_by_label(self, label, include_properties=False):
        return self._yield_nodes(self.node_index.get("label", unicode(label)),
                                 include_properties)
    ...
    def get_relationship_label(self, id):
        edge = self._get_edge(id)
        return self._get_element_label(edge)

    def set_relationship_label(self, id, label):
        edge = self._get_edge(id)
        self._set_element_label(edge, label)
    ...
    def get_relationship_property(self, id, key):
        edge = self._get_edge(id)
        return self._get_element_property(edge, key)

    def set_relationship_property(self, id, key, value):
        edge = self._get_edge(id)
        self._set_element_property(edge, key, value, element_type=EDGE)

    def delete_relationship_property(self, id, key):
        edge = self._get_edge(id)
        self._delete_element_property(edge, key, element_type=EDGE)

    def get_relationship_properties(self, id):
        edge = self._get_edge(id)
        return self._get_element_properties(edge)

    def set_relationship_properties(self, id, properties):
        edge = self._get_edge(id)
        self._set_element_properties(edge, properties, element_type=EDGE)

    def update_relationship_properties(self, id, properties):
        edge = self._get_edge(id)
        self._update_element_properties(edge, properties, element_type=EDGE)

    def delete_relationship_properties(self, id):
        edge = self._get_edge(id)
        self._delete_element_properties(edge, element_type=EDGE)

    def get_relationship_source(self, id, include_properties=False):
        edge = self._get_edge(id)
        vertex = edge.getOutVertex()
        if include_properties:
            return (vertex.getId(), self._get_element_properties(vertex))
        else:
            return (vertex.getId(), None)

    def set_relationship_source(self, relationship_id, node_id):
        v1 = self._get_vertex(node_id)
        edge = self._get_edge(relationship_id)
        v2 = edge.getInVertex()
        label = edge.getLabel()
        self.create_relationship(v1.getId(), v2.getId(), label,
                                 self._get_element_properties(edge))
        self.gdb.removeEdge(edge)
    ...
    def _yield_relationships(self, relationships, include_properties=False):
    ...
    def get_all_relationships(self, include_properties=False):
        return self._yield_relationships(
    ...
    def get_relationships_by_label(self, label, include_properties=False):
        return self._yield_relationships(self.relationship_index.get(
